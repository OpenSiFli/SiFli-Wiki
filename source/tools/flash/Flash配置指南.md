...
![alt text](./assets/nor flash.png)<br>
...
![alt text](/Users/bakac/Workspace/sifli/opensifli/sftool-gui/docs/src/Feature/images/image-5.png)<br>
...
超过128Mbit的flash为了兼容原有MCU boot ROM代码，芯片出厂默认是3字节模式(可访问128Mbit内的内容)，不同厂商对 4 字节地址的支持方式不同。

**有进入退出四字节地址模式指令**

Flash上电默认处于3字节模式，通过发送：

- B7h → 进入4B模式
- E9h → 退出4B模式

进入4B模式后，所有命令的地址字段都变为4字节，命令opcode不变
...
Flash始终处于3字节模式，对于4字节访问使用专用的4B命令（6Ch），3B/4B使用不同的opcode
...
QE bit（Quad Enable bit）是Quad Enable的缩写，串行NAND Flash中的一个重要概念，中文称为四线使能。在串行NAND Flash中，QE bit用于控制引脚的功能复用。具体来说，QE bit决定了Pin3和Pin7的功能：当QE bit使能时，这些引脚用于数据传输；当QE bit不使能时，这些引脚则用于WP#（写保护）、HOLD#（保持）等控制功能**。**
...
typedef struct FLASH_FULL_CHIP_ID
{
    ...
    uint8_t device_id1;  /* 区分同一厂商的不同存储芯片型号 */
    uint8_t device_id2; /* 区分同一厂商的不同存储芯片型号 */
...
在上一节中，介绍了ChipID的组成，实际上ChipID的manufacture_id1、device_id2是9Fh指令所读到的数据。

在标准JEDEC规范下，该数据通常是三个字节，Byte0表示JEDEC分配的厂商码，第二和第三字节用于描述设备型号，例如

- Infineon/Cypress的NOR Flash手册中明确将输出划定为Manufacturer ID、Memory Type、Capacity三段；

- 在Winbond中被表述为1字节厂商+2字节设备信息

但也不乏一些较为老旧的设备型号，如案例二中的东芝Flash，9Fh只有两字节的数据。

一般情况下，可直接在Flash的数据手册中搜索如下关键词，找到数据
