...
- 部分开发板如黄山派、SF32LB52-DevKit-Nano-N16R16等，通过板载串口的RTS管脚复位，主要是方便使用者复位板子（我们新的SifliTrace工具已经添加了RTS复位功能）。
Q:这种设计会带来一种困扰，在板子重新上电后，第一次通过工具连接串口时会触发板子复位，或者在使用过程中出现打开串口板子关机的情况，可以通过如下方式关闭硬件流控解决这个问题。
A:打开设备管理器->右键端口属性->选择端口设置（高级）->勾选禁用modem流控。
